...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        await Api.GetPetServicesCount(query)
            .then(async (res) => {
                if (res.status) {
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetPetServiceSingle({ ServiceId: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/PetServices/edit/${id}`;
        if (type === 'view') _route = `/PetServices/view/${id}`;
                ...
                <Box style={{ width: '100%', paddingBottom: 5 }}>
                    ...
                    <Stack direction="row">
                        <Grid container sx={{ justifyContent: 'flex-end' }}>
                                ...
                                onClick={() => NavigateTo("/PetServices/create")}
                            >
                                <AddBoxIcon />
                            </IconButton>
                        </Grid>
